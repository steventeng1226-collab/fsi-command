...
// SETTINGS TAB
// ═══════════════════════════════════════════════════════════════
function SettingsTab({ sentences, updateSentences, vocab, updateVocab, settings, updateSettings }) {
  const [key, setKey] = useState(settings?.apiKey ?? '')
  const [url, setUrl] = useState(settings?.sheetUrl ?? '')
  const [showKey, setShowKey] = useState(false)
  const [syncing, setSyncing] = useState(false)
  const [msg, setMsg] = useState('')
  const [showSentImport, setShowSentImport] = useState(false)
  const [sentJson, setSentJson] = useState('')
  const sentFileRef = useRef(null)

  function handleSentFile(e) {
    const file = e.target.files?.[0]
    if (!file) return
    const reader = new FileReader()
    reader.onload = ev => setSentJson(ev.target.result)
    reader.readAsText(file)
  }

  function importSentJson() {
    try {
      const arr = JSON.parse(sentJson)
      if (!Array.isArray(arr)) throw new Error('必須是陣列格式')
      const normalized = arr.map((s, i) => ({
        id: s.id ?? `imp_s${Date.now()}_${i}`,
        mode: s.mode ?? 'simple',
      ...
      if (!normalized.length) throw new Error('找不到有效句子')
      updateSentences(prev => {
        const existingIds = new Set((prev??[]).map(s => s.id))
        const newItems = normalized.filter(s => !existingIds.has(s.id))
        return [...(prev??[]), ...newItems]
      })
      flash(`✓ 匯入 ${normalized.length} 個句子完成！`)
      setSentJson(''); setShowSentImport(false)
    } catch(e) {
      flash('✗ 格式錯誤：' + e.message)
    }
  }
  ...
  function handleVocabFile(e) {
    const file = e.target.files?.[0]
    if (!file) return
    const reader = new FileReader()
    reader.onload = ev => setVocabJson(ev.target.result)
    reader.readAsText(file)
  }

  function importVocabJson() {
    try {
      const arr = JSON.parse(vocabJson)
      if (!Array.isArray(arr)) throw new Error('必須是陣列格式 [ {...}, {...} ]')
      const normalized = arr.map((v, i) => ({
        id: v.id ?? `imp${Date.now()}_${i}`,
        word: v.word ?? '',
        ipa_us: v.ipa_us ?? v.ipa ?? '',
        def: v.def ?? v.meaning ?? v.definition ?? '',
        ex: v.ex ?? v.example ?? '',
        reps: 0, ease: 2.5, interval: 1, dueDate: 0, lastSeen: 0
      })).filter(v => v.word)
      if (!normalized.length) throw new Error('找不到有效單字，請確認格式')
      // Merge: keep existing words not in import, add new ones
      updateVocab(prev => {
        const existingWords = new Set((prev??[]).map(v => v.word.toLowerCase()))
        const newWords = normalized.filter(v => !existingWords.has(v.word.toLowerCase()))
        return [...(prev??[]), ...newWords]
      })
      flash(`✓ 匯入 ${normalized.length} 個單字完成！`)
      setVocabJson(''); setShowVocabImport(false)
    } catch(e) {
      flash('✗ JSON 格式錯誤：' + e.message)
    }
  }
  ...
  return (
    <div style={{ padding:'16px 16px 0', display:'flex', flexDirection:'column', gap:18 }} className="fadeUp">
      ...
      {/* Sentence JSON Import */}
      <div style={{ display:'flex', flexDirection:'column', gap:7 }}>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
          <label style={{ fontFamily:MONO, fontSize:9, color:T.txt2, letterSpacing:'0.1em' }}>SENTENCE BULK IMPORT</label>
          <span style={{ fontFamily:MONO, fontSize:9, color:T.amber }}>{(sentences??[]).length} sentences</span>
        </div>
        {!showSentImport ? (
          <button className="btn" onClick={()=>setShowSentImport(true)}
            style={{ background:T.blueD, border:`1px solid ${T.blue}50`, color:T.blue }}>
            📥 IMPORT SENTENCE JSON
          </button>
        ) : (
          <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
            <div style={{ background:T.surf2, borderRadius:9, padding:12 }}>
              <div style={{ fontFamily:MONO, fontSize:9, color:T.blue, marginBottom:6 }}>選擇 JSON 檔案 或 直接貼上</div>
              <input type="file" accept=".json" ref={sentFileRef} onChange={handleSentFile}
                style={{ fontFamily:MONO, fontSize:10, color:T.txt2, marginBottom:8, width:'100%' }}/>
              <textarea value={sentJson} onChange={e=>setSentJson(e.target.value)}
                placeholder={'[\n  {\n    "mode": "simple",\n    "context": "Issue",\n    "hint": "The main issue is low yield.",\n    "template": "The main issue is {blank}.",\n    "subs": [["yield","cost","delay"]]\n  }\n]'}
                style={{ minHeight:100, fontSize:10, lineHeight:1.5 }}/>
            </div>
            <div style={{ display:'flex', gap:8 }}>
              <button className="btn" onClick={importSentJson} disabled={!sentJson.trim()}
                style={{ flex:1, background:T.blue, color:T.bg }}>
                ✓ IMPORT
              </button>
              <button className="btn" onClick={()=>{setShowSentImport(false);setSentJson('')}}
                style={{ background:T.bdr, color:T.txt2, padding:'10px 14px' }}>
                ✕
              </button>
            </div>
          </div>
        ...
        <div style={{ fontFamily:MONO, fontSize:9, color:T.txt3, lineHeight:1.6 }}>
          支援 JSON 格式，欄位：mode、context、hint、template、subs
        </div>
      </div>
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:7 }}>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
          <label style={{ fontFamily:MONO, fontSize:9, color:T.txt2, letterSpacing:'0.1em' }}>VOCAB BULK IMPORT</label>
          <span style={{ fontFamily:MONO, fontSize:9, color:T.amber }}>{(vocab??[]).length} words stored</span>
        </div>
        {!showVocabImport ? (
          <button className="btn" onClick={()=>setShowVocabImport(true)}
            style={{ background:T.amberD, border:`1px solid ${T.amber}50`, color:T.amber }}>
            📥 IMPORT VOCAB JSON
          </button>
        ) : (
          <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
            <div style={{ background:T.surf2, borderRadius:9, padding:12 }}>
              <div style={{ fontFamily:MONO, fontSize:9, color:T.amber, marginBottom:6 }}>選擇 JSON 檔案 或 直接貼上內容</div>
              <input type="file" accept=".json" ref={fileRef} onChange={handleVocabFile}
                style={{ fontFamily:MONO, fontSize:10, color:T.txt2, marginBottom:8, width:'100%' }}/>
              <textarea value={vocabJson} onChange={e=>setVocabJson(e.target.value)}
                placeholder={'[\n  {\n    "word": "yield",\n    "ipa_us": "/jiːld/",\n    "def": "良率",\n    "ex": "Yield improved to 98%."\n  }\n]'}
                style={{ minHeight:100, fontSize:10, lineHeight:1.5 }}/>
            </div>
            <div style={{ display:'flex', gap:8 }}>
              <button className="btn" onClick={importVocabJson} disabled={!vocabJson.trim()}
                style={{ flex:1, background:T.amber, color:T.bg }}>
                ✓ IMPORT
              </button>
              <button className="btn" onClick={()=>{setShowVocabImport(false);setVocabJson('')}}
                style={{ background:T.bdr, color:T.txt2, padding:'10px 14px' }}>
                ✕
              </button>
            </div>
        ...
        <div style={{ fontFamily:MONO, fontSize:9, color:T.txt3, lineHeight:1.6 }}>
          支援 JSON 格式，欄位：word、ipa_us、def、ex<br/>
          重複單字會自動跳過，不覆蓋已有記錄
        </div>
      </div>
